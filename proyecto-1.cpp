    ...
    if(Leer.is_open())
    {
        ...
        while (!Leer.eof())
        {
            for(int i=0;i<5;i++)
            {
                ...
                if(temporal1.escogidos[i].nombre==Factu.escogidos[i].nombre && aux==true && cad2 != cad1)
                {
                    temporal1.escogidos[i].cantidad_compra+=temporal1.escogidos[i].cantidad_compra;   
                }                               
            }
            Leer.read((char *)&Factu,sizeof(Factu));
        }    
